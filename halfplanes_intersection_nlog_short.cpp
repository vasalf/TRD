...
Response intersect_halfs(vector<Line> halfs) {
  ...
  int n = (int)halfs.size(), is_inf = 0,
      any_positive_vm = 0;
  deque<Line> hull;
  Line L1, L2, L3;
  ld vm, sm;
  for (int i = 0; i < n; i++) {
    vm = halfs[i].dir().vector_mul(
      halfs[i + 1 < n ? i + 1 : 0].dir());
    sm = halfs[i].dir().scalar_mul(
      halfs[i + 1 < n ? i + 1 : 0].dir());
...
ld calculate_area(Response response) {
  ...
  for (int i = 0; i < n; i++) {
    int j = i + 1 < n ? i + 1 : 0;
    if (abs(halfs[i].dir().vector_mul(
          halfs[j].dir())) < eps)
      continue;
    ps.push_back(
      intersect_lines(halfs[i], halfs[j]).second);
  }
  n = (int)ps.size();
  for (int i = 0; i < n; i++) {
    int j = i + 1 < n ? i + 1 : 0;
